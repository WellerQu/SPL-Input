...
import { useSyntaxSuggestions, DistinctField } from './hooks/useSyntaxSuggestions'
...
export const QueryInput: React.FC<QueryInputProps> = props => {
  ...
  const onQueryChange = useMemo(() => props.onQueryChange, [props.onQueryChange])
  const onQueryEnter = useMemo(() => props.onQueryEnter, [props.onQueryEnter])

  const [showIntelliSense, setShowIntelliSense] = useState(false)
  const [completionItems, refactorUserInput] = useSyntaxSuggestions(value, props.fieldOptionItems)
  const [autoFocus , setAutoFocus] = useState(false)

  const handleQueryChange = useCallback((e: React.FormEvent<HTMLInputElement>) => {
    onQueryChange && onQueryChange(refactorUserInput(e.currentTarget.value))
  }, [onQueryChange, refactorUserInput])

  const handleQueryTab = useCallback((item: CompletionItem | undefined) => {
    item && onQueryChange && onQueryChange(refactorUserInput(item.label, true))
    ref.current?.focus()
  }, [onQueryChange, refactorUserInput])
