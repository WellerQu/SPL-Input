...
const ProviderMenuStyle = styled(Menu)`
  ...
  border-width: 1px;
  border-style: solid;
  box-shadow:  0 3px 6px -4px rgba(0, 0, 0, 0.12), 0 6px 16px 0 rgba(0, 0, 0, 0.08), 0 9px 28px 8px rgba(0, 0, 0, 0.05); /* ${props => props.theme["box-shadow-base"]}; */
`

const DescStyle = styled.span`
  color: rgba(0, 0, 0, 0.45); /* ${props => props.theme["text-color-secondary"]}; */
`
...
const ProviderMenu = ({
  ...
  return (
    <>
      ...
      {dataSource.length === 0 &&
        <Card size='small'>
          <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description='无提示项, 请检查语法是否正确' />
        </Card>
      }
    </>
  )
 ...
 * 提供智能感知的 Input 输入框
 */
export const QueryCompletionProvider = React.forwardRef<Input, CompletionProviderType>((props, ref) => {
  const { dataSource, loading, visible = false, onQueryInputFocus, onQueryInputBlur, onCompletionSelect, ...rest } = props
  const [selectedItem, onKeyEvent] = useCompletionFocus<CompletionItem>(dataSource, onCompletionSelect)
  const className = useMemo(() => `special-tag-${uuidv4()}`, [])

  const getContainer = useCallback(() => document.querySelector(`.${className}`) as HTMLElement, [className])

  // 通过鼠标选择备选项
  const handleProviderSelect = useCallback((item: CompletionItem) => {
